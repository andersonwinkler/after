...
DOSEGPVE=false          # Compute partial volume effects
DOSMOOTH=false          # Compute pairwise distance between vertices
FWHM=5                  # Default FWHM for smoothing (if --smooth is not specified, but --all is)
...
while [[ ${#} -gt 0 ]] ; do
   ...
   -myelin|--myelin)
      ...
      shift
      ;;
   -subregions|--subregions|--subseg|--subsegmentations)
      DOSUBSEG=true
      shift
      ;;
   -segpve|--segpve)
...
# We do not want to overwrite when importing new data
...
# We do not want to overwrite in --sdnew (would defeat the purpose)
...
if ${DORETESSELLATE} || ${DOALL} ; then
   ...
   ${T2pial}    && OPTS="${IN_T2W} -T2pial"
   ${FLAIRpial} && OPTS="${IN_FLA} -FLAIRpial"
...
if ${DOSUBSEG} || ${DOALL} ; then
   ...
   ${FREESURFER_HOME}/bin/mri_sclimbic_seg --s "${subj}" --sd "${SUBJECTS_DIR}"
   ${FREESURFER_HOME}/bin/mri_segment_hypothalamic_subunits --s "${subj}" --sd "${SUBJECTS_DIR}"
fi

# =====================================================================
# Compute partial volume effects for segmented structures
if ${DOSEGPVE} || ${DOALL} ; then
   if ${DOMYELIN} || ${DOALL} ; then
      OPTS="--myelin"
   else
      OPTS=""
   fi
   ${AFTERDIR}/segpve --subj ${subj} --subjdir "${SUBJECTS_DIR}" ${OPTS}
fi

# =====================================================================
# Make nice figures and histograms for visual inspection for known data
# Known data that are missing will be skipped. Views already generated will
# also be skipped; to regenerate for these, delete the previous files.
if ${DOVIEWS} || ${DOALL} ; then
   ${AFTERDIR}/views --subj ${subj} --subjdir "${SUBJECTS_DIR}" --all
fi
...
if ${DOSMOOTH} || ${DOALL} ; then
   ...
   # Create the retessellated midthickness surface if for some reason it wasn't created earlier
   for h in lh rh ; do
      if [[ ! -f ${SUBJECTS_DIR}/${subj}/surf/${h}.midthickness ]] ; then
         ${FREESURFER_HOME}/bin/mris_expand \
                    -thickness ${SUBJECTS_DIR}/${subj}/surf/${h}.white  \
   ...
   # Do the smoothing proper
   ${AFTERDIR}/smooth --method fs --subj ${subj} --fwhm ${FWHM}

   # Make views for the smoothed data (views already made will be skipped)
   if ${DOVIEWS} || ${DOALL} ; then
      ${AFTERDIR}/views --subj ${subj} --subjdir "${SUBJECTS_DIR}" --all --fwhm ${FWHM}
   fi
fi
